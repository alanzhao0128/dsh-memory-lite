...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const FIELDS = [
      ...
      { path: ['extraction', 'messageScope'], type: 'multicheck', group: 'extraction', sub: '提取内容', label: '计入窗口的消息类型', options: [['user', '用户消息'], ['assistant', '助手消息'], ['tool_result', '工具结果原文']], hint: '勾选的类型计入触发计数并进入提取窗口；工具结果默认排除（助手消息已含解读，原文只增成本与噪声）。三项全不勾 = 用默认（用户 + 助手）。保存即时生效。' },
    ...
    // Keep a stored value visible when its option disappears. A provider model
    // can be renamed or deleted while the route (or a reasoning effort) stays
    // pinned in settings; without this the select silently falls back to its
    // first entry, so the page looks healthy while extraction keeps calling a
    // dead model (2026-09-16: route huoshan/deepseek-v4-flash rendered as
    // follow-global-default).
    //   list      - current options, the '' default first.
    //   current   - stored (or drafted) value.
    //   listReady - whether list is authoritative: the catalog/effort fetch
    //               already answered. While it has not, the stored value is
    //               shown as-is and never flagged.
    const withStoredValue = (list, current, listReady, label) => {
      const options = list || [];
      const value = current === undefined || current === null ? '' : String(current);
      if (value === '') return { options, stale: false };
      if (options.some((opt) => String(opt[0]) === value)) return { options, stale: false };
      const head = listReady ? [value, label(value)] : [value, value];
      return { options: [head, ...options], stale: listReady };
    };
    ...
    const warningStyle = { fontSize: 11, lineHeight: '16px', color: 'var(--dsw-alias-state-error-primary)', marginTop: 3 };
    const inputStyle = {
    ...
    function useModelOptions(connection, agentDefaultScope, selectedRoute) {
      ...
      // Reasoning efforts belong to one concrete model route. An empty
      // selection means 跟随全局默认: that route IS the live global default
      // model, so the dropdown must offer its efforts, not just the empty
      // option (which is what the field hint already promises).
      const selectedKey = typeof selectedRoute === 'string' ? selectedRoute : '';
      const effortRouteTarget = selectedKey === '' ? defRoute : selectedKey;

      // Resolved lazily per target route through /memory-model-efforts (host
      // resolveModelInfo), not embedded in the catalog (N host calls/mount).
      // effortRoute remembers which route the fetched `efforts` describe.
      useEffect(() => {
        if (!connection || effortRouteTarget === '') { setEfforts(null); setEffortRoute(''); return; }
        let alive = true;
        const load = async () => {
          try {
            const res = await connection.rpc.call('/api', 'memory-model-efforts/snapshot', { route: effortRouteTarget });
            if (!alive) return;
            setEffortRoute(effortRouteTarget);
            setEfforts(res && res.ok && res.value ? res.value : { efforts: [], defaultEffort: undefined });
          } catch (err) {
            if (!alive) return;
      ...
      // Options for the effort dropdown: the efforts of the route the value
      // belongs to, where '' means the live global default model (resolved
      // above). While that route's fetch is in flight only the default shows.
      const effortOptionsFor = (route) => {
        const opts = [['', '跟随全局默认']];
        const target = route === undefined || route === null || route === '' ? defRoute : String(route);
        if (target === '' || target !== effortRoute) return opts;
      ...
      // A list is authoritative only once its fetch answered; until then a
      // stored value missing from the options is not yet known to be stale.
      const catalogReady = catalog !== null && !failed;
      const effortsReadyFor = (route) => {
        const target = route === undefined || route === null || route === '' ? defRoute : String(route);
        return target !== '' && target === effortRoute && efforts !== null;
      };
      return { routeOptions, effortOptionsFor, failed, catalogReady, effortsReadyFor };
    }
    ...
    function FieldRow({ spec, value, draft, onChange, modelOptions }) {
      ...
      let control;
      // Set by the dynamic select below when the stored value is missing from
      // its option list (the value is then also shown, flagged, as an option).
      ...
      if (spec.type === 'toggle') {
        ...
        // Either list can stop containing the stored value (a provider model is
        // renamed or deleted): withStoredValue keeps that value on screen and
        // flags it instead of letting the control fall back to its first entry.
        let options;
        let listReady;
        let staleLabel;
        let isRoute = false;
        if (spec.dynamic === 'route') {
          ...
          listReady = modelOptions ? modelOptions.catalogReady : false;
          staleLabel = (v) => v + '（已失效：provider 中已无此模型）';
          isRoute = true;
        } else {
          ...
          listReady = modelOptions && modelOptions.effortsReadyFor ? modelOptions.effortsReadyFor(routeVal) : false;
          staleLabel = (v) => v + '（已失效：所选模型未声明该推理强度）';
        }
        const resolved = withStoredValue(options, shown, listReady, staleLabel);
        staleWarning = resolved.stale
          ? (isRoute
            ? '当前保存的模型已不在 provider 的模型列表中（多半是被改名或删除了）；提取仍会用它并持续失败，请重新选择。'
            : '当前保存的推理强度不在所选模型的可用列表中；提取仍会用它。')
          : null;
        control = jsx('select', {
          value: String(shown ?? ''),
          // The lazy efforts fetch follows the selected route on its own
          // (useModelOptions takes it as an argument), so nothing to re-target.
          onChange: (e) => { onChange(key, e.target.value); },
          style: inputStyle,
          children: (resolved.options || []).map(([optValue, optLabel]) =>
            jsx('option', { value: optValue, children: optLabel, key: optValue })),
        });
      ...
      return jsx('div', { style: rowStyle, children: [
        ...
        control,
        staleWarning === null ? null : jsx('div', { style: warningStyle, children: staleWarning }),
    ...
    function SettingsPage({ scope, connection, agentDefaultScope }) {
      ...
      // The route the effort dropdown belongs to: the staged edit wins, then
      // the stored value. '' = 跟随全局默认, which useModelOptions resolves to
      // the live default model for the efforts fetch.
      const draftRoute = draft['extraction.llm.route'];
      const storedRoute = readPath(value, ['extraction', 'llm', 'route']);
      const selectedRoute = draftRoute !== undefined
        ? (typeof draftRoute === 'string' ? draftRoute : '')
        : (typeof storedRoute === 'string' ? storedRoute : '');

      const modelOptions = useModelOptions(connection, agentDefaultScope, selectedRoute);
      ...
      const fieldValue = (spec) => {
        ...
        const chosen = draft[key] !== undefined ? draft[key] : readPath(value, spec.path);
        if (chosen === undefined) return fallback;
        // The host resolves an empty multicheck to its default
        // (normalizeMessageScope: [] -> user+assistant), so the panel has to
        // show that same selection — otherwise it renders "nothing checked"
        // while the runtime keeps counting those kinds.
        if (Array.isArray(chosen) && chosen.length === 0 && Array.isArray(fallback)) return fallback;
        return chosen;
      };
      ...
      const save = async () => {
        ...
        try {
          const ops = Object.entries(draft).map(([key, val]) => {
            ...
            // An all-unchecked multicheck means "use the default" on the host,
            // so persist an unset rather than an empty array in the document.
            if (spec && spec.type === 'multicheck' && Array.isArray(val) && val.length === 0) {
              return { op: 'unset', path };
            }
