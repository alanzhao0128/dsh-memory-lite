...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function useModelOptions(connection, agentDefaultScope) {
      ...
      const effortsReadyFor = (route) => route !== '' && route === effortRoute && efforts !== null;
      return { routeOptions, effortOptionsFor, selectRoute, failed, catalogReady, effortsReadyFor };
    }
    ...
    function SettingsPage({ scope, connection, agentDefaultScope }) {
      ...
      // Fetch the stored route's reasoning efforts on mount. Without this the
      // effort dropdown only offers 跟随全局默认 until the user re-picks the
      // route, which hides both the model's real efforts and a stale one.
      const storedRoute = readPath(value, ['extraction', 'llm', 'route']);
      const storedRouteKey = typeof storedRoute === 'string' ? storedRoute : '';
      useEffect(() => {
        if (storedRouteKey === '' || !modelOptions || !modelOptions.selectRoute) return;
        modelOptions.selectRoute(storedRouteKey);
      }, [storedRouteKey]);
