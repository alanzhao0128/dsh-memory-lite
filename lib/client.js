...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const GROUPS = [
      { id: 'extraction', label: '提取（extraction）' },
      { id: 'index', label: '索引（index）' },
      { id: 'ui', label: '界面（ui）' },
      { id: 'general', label: '常规（重启生效）' },
    ];

    const FIELDS = [
      { path: ['extraction', 'mode'], type: 'select', group: 'extraction', label: '提取模式', options: [['incremental', 'incremental（增量）'], ['explicit_only', 'explicit_only（仅显式）'], ['off', 'off（关闭）']], hint: '增量提取开关；explicit_only/off 时指示器置灰' },
      { path: ['extraction', 'windowTurns'], type: 'number', group: 'extraction', label: '触发窗口（消息数）', hint: '新增 surface 消息数 ≥ 此值触发一次提取；0 = 每条消息立即触发（非禁用）' },
      { path: ['extraction', 'idleTimeoutMin'], type: 'number', group: 'extraction', label: '空闲兜底（分钟）', hint: '空闲 N 分钟后提取剩余窗口；0 = 空闲立即提取（非禁用）' },
      { path: ['extraction', 'maxMessages'], type: 'number', group: 'extraction', label: '单次消息上限', hint: '单次提取喂给 LLM 的最大消息数；0 = 窗口为空，不提取（等效禁用）' },
      { path: ['extraction', 'toolResultMaxBytes'], type: 'number', group: 'extraction', label: '工具结果截断（字节）', hint: '每条 tool_result 喂给 LLM 前的截断；0 = 工具内容全部丢弃' },
      { path: ['extraction', 'minTurnExtract'], type: 'number', group: 'extraction', label: '回合边界最小消息数', hint: 'turn-stopping/flush 触发的未提取消息下限；0 = 无条件触发' },
      { path: ['extraction', 'turnDebounceMs'], type: 'number', group: 'extraction', label: '回合防抖（毫秒）', hint: 'turn-boundary 提取的防抖窗口；0 = 无防抖，每次都触发' },
      { path: ['extraction', 'maxConcurrentRequests'], type: 'number', group: 'extraction', label: '并发提取上限', hint: '预留字段，当前未接线（改动不生效）' },
      { path: ['extraction', 'includeDigest'], type: 'toggle', group: 'extraction', label: '携带会话摘要', hint: '提取时携带滚动会话 digest' },
      { path: ['extraction', 'dedup'], type: 'toggle', group: 'extraction', label: '去重检索', hint: '提取前 grep 已有记忆做去重/矛盾判断' },
      { path: ['extraction', 'turnStoppingTrigger'], type: 'toggle', group: 'extraction', label: '回合边界触发', hint: 'agent/turn-stopping 时触发提取（成本较高）' },
      { path: ['extraction', 'flushTrigger'], type: 'toggle', group: 'extraction', label: '会话落盘触发', hint: 'session/flush 时触发提取' },
      { path: ['extraction', 'parseRetry'], type: 'toggle', group: 'extraction', label: '解析失败重试', hint: 'JSON 解析失败时一次廉价 repair 调用' },
      { path: ['extraction', 'auditLog'], type: 'toggle', group: 'extraction', label: '审计日志', hint: '写 peers/{peer}/sessions/{id}.json' },
    ...
    function FieldRow({ spec, value, draft, onChange }) {
    ...
    function SettingsPage({ scope, connection }) {
      ...
      const fieldValue = (spec) => {
        ...
        return readPath(value, spec.path);
      };
      ...
      const groups = GROUPS.map((group) => {
        ...
        return jsx('div', { key: group.id, style: { marginBottom: 20 }, children: [
          ...
          ...fields.map((spec) => jsx(FieldRow, {
            key: pathKey(spec.path),
            spec,
            value: readPath(value, spec.path),
            draft,
            onChange: setField,
          })),
        ]});
