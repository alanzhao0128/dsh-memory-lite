...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const FIELDS = [
      ...
      // ---- 提取模型 ----
      { path: ['extraction', 'llm', 'route'], type: 'dynamic-select', group: 'extraction', sub: '提取模型', label: '提取模型', hint: '提取运行的模型；跟随全局默认 = 用 agent-default-model 当前值（活）' },
      { path: ['extraction', 'llm', 'reasoningEffort'], type: 'dynamic-select', group: 'extraction', sub: '提取模型', label: '推理强度', hint: '跟随全局默认 = 用默认模型的推理强度；选项按所选模型的配置读取' },
      // ---- 可靠性 ----
